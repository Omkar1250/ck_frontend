...
const ActivationRequest = () => {
  ...
  } = useSelector((state) => state.activationRequests);
  ...
  const handleActivationApproval = async () => {
    try {
      await activationAction(token, selectedLead?.id, modalAction);
      toast.success(
  ...
  const filteredLeads = useMemo(
    ...
    [activationRequests, searchQuery]
  );
  ...
  return (
    <div className="max-w-6xl mx-auto mt-24 px-4 sm:px-6 lg:px-8">
      <h2 className="text-3xl font-bold mb-4 text-center text-gray-800">
        Activation Requests ({totalActivationRequests})
      </h2>
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={closeModal}
        onSubmit={handleActivationApproval}
        ...
        action={modalAction}
      >
        {selectedLead?.activation_screenshot && (
          <div className="flex flex-col items-center justify-center mt-4">
            <button
              onClick={() => {
                setModalImage(`${IMAGE_URL}/${selectedLead?.activation_screenshot}`);
