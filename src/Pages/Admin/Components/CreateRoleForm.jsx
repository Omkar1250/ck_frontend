...
export default function CreateRoleForm({ closeModal, rm }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        toast.success("JRM updated successfully");
      } else {
        await createRm(token, formData);
        toast.success("JRM created successfully");
      }
      closeModal();
    } catch (error) {
      toast.error(`Failed to ${isEdit ? "update" : "create"} JRM`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.success("JRM deleted successfully");
      closeModal();
    } catch (error) {
      toast.error(error.message || "Failed to delete JRM");
    } finally {
      setLoading(false);
    }
  };
