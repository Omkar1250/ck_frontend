// src/Pages/Admin/AdminPendingSipRequests.jsx
...
// ✅ Redux slice + API actions (you already have these from previous steps)
import {
  setAdminSipRequestsPage,
} from "../../Slices/adminSlices/adminSipRequestsSlice";
import {
  getPendingSipRequests,
} from "../../operations/adminApi"; // GET list
import {
  approveOrRejectSipStageRequest,
  getAllBatchCodes,
} from "../../operations/adminApi"; // Approve/Reject + batches

const IMAGE_URL = process.env.REACT_APP_IMAGE_URL;

// ✅ Stage mapping aligned to your DB structure
const STAGE_META = {
  session4: {
    label: "Session 4",
    statusField: "rm_mf_status_session_4",
    approvalField: "approval_session_4",
    screenshotField: "screenshot_session_4",
  },
  session19: {
    label: "Session 19",
    statusField: "rm_mf_status_session_19",
    approvalField: "approval_session_19",
    screenshotField: "screenshot_session_19",
  },
  batchEnd: {
    label: "Batch End",
    statusField: "rm_mf_status_batch_end",
    approvalField: "approval_batch_end",
    screenshotField: "screenshot_batch_end",
  },
  monthly: {
    label: "Monthly Follow-up",
    statusField: "rm_mf_status_monthly",
    approvalField: "approval_monthly",
    screenshotField: "screenshot_monthly",
  },
};

const STAGE_KEYS = Object.keys(STAGE_META);

// 🔎 helper: find which stage is pending approval & SIP converted for a given lead
function getPendingStage(lead) {
  for (const key of STAGE_KEYS) {
    const { statusField, approvalField, screenshotField, label } = STAGE_META[key];
    if (
      lead?.[approvalField] === "pending" &&
      lead?.[statusField] === "sip_done_converted"
    ) {
      return {
        key,
        label,
        ...
        approvalField,
        screenshotField,
        screenshot: lead?.[screenshotField] || null,
      };
    }
  }
  return null;
...
const AdminPendingSipRequests = () => {
  ...
  const {
    ...
    totalPages,
    totalRequests,
  } = useSelector((state) => state.adminSipRequests);
  ...
  // 🔹 Fetch pending SIP list (server-side pagination)
  useEffect(() => {
    dispatch(getPendingSipRequests(currentPage, 6));
  }, [dispatch, currentPage]);

  // local filter (batch + text)
  const filteredLeads = useMemo(() => {
    return (requests || []).filter((lead) => {
      const matchesBatch = selectedBatch ? lead.batch_code === selectedBatch : true;
  ...
  }, [requests, selectedBatch, searchQuery]);
  ...
  const handleNext = () => {
    if (currentPage < totalPages) {
      ...
      dispatch(setAdminSipRequestsPage(newPage));
      dispatch(getPendingSipRequests(newPage, 6));
    }
  };
  ...
  const openModal = (lead, action) => {
    const pendingStage = getPendingStage(lead);
    if (!pendingStage) {
      toast.error("No pending stage found for this lead.");
      return;
    }
    setSelectedLead(lead);
    setSelectedStageKey(pendingStage.key);
  ...
  const handleApprovalAction = async () => {
    if (!selectedLead?.id || !selectedStageKey) return;
    try {
      await approveOrRejectSipStageRequest(token, selectedLead.id, {
        stage: selectedStageKey,
        action: modalAction,
      });
      // toast is inside your api or do it here:
    ...
    <div className="max-w-7xl mx-auto mt-20 px-4 sm:px-6 lg:px-8">
      ...
      <div className="text-center mb-8">
        <h2 className="text-3xl font-extrabold text-gray-900">
          💹 MF — Pending SIP Converted Requests{" "}
      ...
      {loading ? (
        ...
        <div className="text-center mt-16">
          ...
          <button
            ...
            onClick={() => dispatch(getPendingSipRequests(currentPage, 6))}
          >
            Retry
          </button>
        </div>
      ...
      <Modal
        ...
        title={modalAction === "approve" ? "Approve SIP" : "Reject SIP"}
        action={modalAction}
      >
        {/* View Screenshot (if available on pending stage) */}
        {selectedLead && selectedStageKey && (() => {
          const { screenshotField } = STAGE_META[selectedStageKey];
          const path = selectedLead?.[screenshotField];
          if (!path) return null;
          return (
...
const LeadCard = ({ lead, copyToClipboard, openWhatsApp, makeCall, openModal }) => {
  ...
  // Friendly label for the pending stage
  const stageLabel = pendingStage?.label || "Stage";
  const statusFieldLabel = pendingStage
    ? lead[pendingStage.statusField]?.replaceAll("_", " ")
    : "—";
  const screenshotPath = pendingStage?.screenshot || null;
    ...
    <div className="bg-white border border-gray-100 p-5 rounded-2xl shadow-[0_8px_25px_rgba(0,0,0,0.06)] hover:shadow-[0_12px_30px_rgba(0,0,0,0.10)] hover:-translate-y-[2px] transition-all duration-300">
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Added {lead.created_at ? format(lead.created_at) : "recently"}
          </p>
        </div>
        ...
        {screenshotPath && (
          <button
            onClick={() => {
              const url = `${IMAGE_URL}/${screenshotPath}`;
      ...
      <div className="text-sm text-gray-700 space-y-3">
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="text-xs text-gray-600">
            {stageLabel}:{" "}
            <span className="font-semibold text-gray-800">{statusFieldLabel}</span>
          </span>
        </div>
...
export default AdminPendingSipRequests;
