...
import { markCallDoneOfBasicLead,  mFClientsForCall,  mFMarkCallDone,  newBasicClientsForCall } from "../../operations/rmApi";

const MfClients = () => {
  const dispatch = useDispatch();
  const { token } = useSelector((state) => state.auth);
  const { mfClients = [], loading, error, currentPage, totalPages,totalMfClients } = useSelector(
    (state) => state.mfClients
  );
  ...
  useEffect(() => {
    dispatch(mFClientsForCall(currentPage || 1, 5, searchQuery));
    ...
    const handleNext = () => {
      if (currentPage < totalPages) {
        ...
        dispatch(mFClientsForCall(newPage, 5, searchQuery)); // Fetch leads for new page
      }
    };
  ...
  const handleSendMsDetails = async () => {
    ...
    try {
    const res=  await mFMarkCallDone(token,selectedLead.id, {action: 'approve'});
      toast.success("Marked As Call Done!");
      dispatch(mFClientsForCall(currentPage, 5, searchQuery));
      closeFormModal();
    } catch (error) {
      toast.error("Failed!");
      console.error(error)
  ...
  return (
    ...
    <p className="text-center text-2xl font-bold font-mono">Mf Clients  <span className="text-btnColor">{totalMfClients}</span></p>
      <div className="mb-4">
        <SearchInput
          value={searchQuery}
